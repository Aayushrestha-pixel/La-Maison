...
// optional fallback (used only if API fails)
const mockBookings = [
  { id: 1, name: 'Aayush Sharma', email: 'aayush@email.com', phone: '9841234567', date: '2025-08-10', time: '19:00', guests: 2, occasion: 'anniversary', status: 'confirmed' },
]
...
function Dashboard() {
  ...
  //     // normalize id just in case (_id → id)
  ...
  //   } catch (err) {
  //     console.error('Failed to fetch bookings:', err)
  //     setBookings(mockBookings) // fallback only if API fails
  //   } finally {
  //     setLoading(false)
  //   }
  // }
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      ...
      console.error('status update failed', err)
      setBookings(prev) // revert if failed
    }
  }
